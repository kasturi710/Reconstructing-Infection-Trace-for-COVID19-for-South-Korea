...
noise = 100
model = 'SI'
inf_fraction = float(sys.argv[1])*0.01
#TS, snapshots, G = readFile('generated.txt')

TS, G, infection_order, infection_track, seeds, nodes = get_noisy_TS_leaves_with_P(type = dataset, n = N, inf_fraction = inf_fraction, seeds_num = srcN, noise = noise, reportP = reportingP, model = model)
K = len(seeds)
...
print 'timing:'
print inf_fraction, SPtime+BStime, SPtime, BStime
