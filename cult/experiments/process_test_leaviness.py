...
path = os.path.join('CONDOR', 'leaviness_loop', 'res')

print path
onlyfiles = [f for f in listdir(path) if isfile(join(path, f)) and dataset in f]
print onlyfiles
...
for f in onlyfiles:
    ...
    #print fullname
    l_range, correct_found_seeds, cost_ratio, correct_interaction_recall, correct_interaction_precision, \
...
plt.plot(l_range, np.nanmean(arr_correct_found_seeds, axis=0))
plt.xlabel('percent of leaves')
...
plt.xlabel('percent of leaves')
plt.plot(l_range, np.nanmean(arr_cost_ratio, axis=0))
plt.xlabel('percent of leaves')
#plt.show()
...
plt.plot(l_range, np.nanmean(arr_correct_causality_precision, axis=0))
plt.plot(l_range, np.nanmean(arr_correct_interaction_precision, axis=0))
plt.plot(l_range, np.nanmean(arr_correct_interaction_recall, axis=0))
plt.plot(l_range, np.nanmean(arr_correct_causality_recall, axis=0))
#plt.plot(l_range, np.nanmean(arr_total_order_tau, axis=0))
#plt.legend(['recall, interactions ', 'precision, interactions', 'recall, order', 'precision, order', 'total order tau'], loc = 3)
#plt.legend(['Static P.', 'Temporal P.',  'Static R.', 'Temporal R.'], loc = 1, fontsize=22)
plt.xlabel('percent of frontier nodes', fontsize=30)
plt.ylim(0.0, 1.0)
plt.tight_layout()
#plt.show()
...
plt.plot(l_range, np.nanmean(arr_mean_slack, axis=0))
...
plt.plot(l_range, np.nanmean(arr_prec_out, axis=0), 'k-')
plt.plot(l_range, np.nanmean(arr_prec_lb, axis=0), 'k--')
plt.plot(l_range, np.nanmean(arr_prec_ub, axis=0), 'k:')
plt.plot(l_range, np.nanmean(arr_recall_out, axis=0), 'r-')
plt.plot(l_range, np.nanmean(arr_recall_lb, axis=0), 'r--')
plt.plot(l_range, np.nanmean(arr_recall_ub, axis=0), 'r:')
...
plt.plot(l_range, np.nanmean(arr_prec_out_05, axis=0), 'k-')
plt.plot(l_range, np.nanmean(arr_prec_lb_05, axis=0), 'k--')
plt.plot(l_range, np.nanmean(arr_prec_ub_05, axis=0), 'k:')
plt.plot(l_range, np.nanmean(arr_recall_out_05, axis=0), 'r-')
plt.plot(l_range, np.nanmean(arr_recall_lb_05, axis=0), 'r--')
plt.plot(l_range, np.nanmean(arr_recall_ub_05, axis=0), 'r:')
...
plt.plot(l_range, np.nanmean(arr_F_out, axis=0))
plt.plot(l_range, np.nanmean(arr_F_lb, axis=0))
plt.plot(l_range, np.nanmean(arr_F_ub, axis=0))
...
plt.plot(l_range, np.nanmean(arr_correct_causality_F, axis=0))
#plt.plot(l_range, np.nanmean(arr_ub_correct_causality_F, axis=0))
plt.plot(l_range, np.nanmean(arr_correct_interaction_F, axis=0))
#plt.plot(l_range, np.nanmean(arr_ub_correct_interaction_F, axis=0))
#plt.legend(['order output', 'order BL', 'interactions output', 'interactions BL'], loc = 3)
plt.legend(['order output', 'interactions output'], loc = 3)
#plt.ylabel('F1')
plt.xlabel('percent of leaves')
plt.tight_layout()
...
plt.plot(l_range, np.nanmean(arr_mcc_out, axis=0))
plt.plot(l_range, np.nanmean(arr_mcc_lb, axis=0))
plt.plot(l_range, np.nanmean(arr_mcc_ub, axis=0))
