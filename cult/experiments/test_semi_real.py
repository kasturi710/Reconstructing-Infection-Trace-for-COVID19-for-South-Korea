...
I = 100
arr_abs_values_tp = [np.nan] * I
arr_abs_values_p = [np.nan] * I
arr_abs_values_lb_tp = [np.nan] * I
arr_abs_gt_positive = [np.nan] * I
arr_abs_values_ub_tp = [np.nan] * I
arr_abs_values_ub_p = [np.nan] * I

arr_prec_infected = [np.nan] * I
arr_prec_lb_infected = [np.nan] * I
arr_prec_ub_infected = [np.nan] * I

arr_recall_infected = [np.nan] * I
arr_recall_lb_infected = [np.nan] * I
arr_recall_ub_infected = [np.nan] * I
...
for iters in xrange(I):
    TS, G, infection_order, infection_track, seeds, fail = generateTS_final(n=N,
                                        p=p,
                                        seed=1.0,
                                        st=datetime(2000, 01, 01, 00, 00, 00),
                                        m=M,
                                        srcN=srcN,
                                        reportingP=reportingP,
                                        infectionP=infectionP,
                                        recoveringP=recoveringP,
                                        type=type,
                                        real=real,
                                        percentage=0.5)
