...
path = os.path.join('CONDOR', 'timing_incremental', 'res')
...
plt.plot(x, time_SP)
plt.plot(x, time_BS)
plt.legend(['total', 'shortest paths', 'binary search'], loc = 2, fontsize=27)
#plt.show()
#exit()
#plt.legend(['MCC', 'MCC reports', 'MCC BL'], loc = 3)
plt.ylabel('running time (sec)', fontsize=27)
plt.xlabel('number of new time-stamped edges', fontsize=27)
plt.tight_layout()
name = plt.savefig('timing_incremental_length_avg.pdf')
exit()
